...
int Cuadricula::newTower() {
    ...
    if (posibleTowerIdList.size() == 0){
        ...
        for (int n = 0; n < 1; n++) {
            ...
            if ((!getNode(posibleTowerIdList[i])->getTorre()) && posibleTowerIdList[i] != 0 &&
                posibleTowerIdList[i] != ((ZONE_SIZE - 1) * (ZONE_SIZE) + (ZONE_SIZE - 1))) {

                getNode(posibleTowerIdList[i])->setTorre(true);
...
void Cuadricula::printTorres() {
    ...
    for (int i = 0 ; i < size ; i++) {
        for (int j = 0 ; j < size ; j++) {
            ...
            if (matriz[i][j]->getTorre() != false && matriz[i][j]->isInAStarPath()) {
                place = "2";
            }
...
void Cuadricula::addPosibleTowerIdList() {
    for(int i=0; i<ZONE_SIZE; i++){
        for (int j=0; j<ZONE_SIZE; j++){
            if (matriz[i][j]->getTorre() == false && matriz[i][j]->getId() != 0 && matriz[i][j]->getId() != ( (ZONE_SIZE-1)*(ZONE_SIZE) + (ZONE_SIZE-1) ) ){
...
void Cuadricula::deleteTower(int evaluatingTower) {
    ...
    getNode(evaluatingTower)->setTorre(false);
}
